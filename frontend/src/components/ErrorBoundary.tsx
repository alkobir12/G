    ...
    return (
      <div dir="rtl" className="p-6 rounded-2xl border border-rose-400/30 bg-rose-500/10 text-slate-100 m-4">
        <h2 className="text-lg font-bold mb-2">
          {this.props.label ? `حدث خطأ في صفحة: ${this.props.label}` : "حدث خطأ في هذه الصفحة"}
        </h2>
        <p className="text-sm text-slate-300 mb-3">
          {this.state.error?.message || "خطأ غير متوقع"}
        </p>
        <button
          data-testid={`error-retry-${this.props.label || "app"}`}
        ...
        <button
          data-testid={`error-reload-${this.props.label || "app"}`}
